 ...
 */
namespace Lof\RequestForQuote\Model\Quote\Item;
...
class Updater extends \Magento\Quote\Model\Quote\Item\Updater
{
    ...
    public function update(Item $item, array $info)
    {
        ...
        if (empty($info['action']) || !empty($info['configured'])) {
            ...
            $item->setQty($itemQty);
            $item->setQtyShowup($itemQty);
            //copy qtyshowup column value to qty
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $resource = $objectManager->get('Magento\Framework\App\ResourceConnection');
            $connection = $resource->getConnection();
            $tableName = $resource->getTableName('quote_item');
            $sql = "update " . $tableName . " set qty = ". $itemQty ." where item_id = ".$item->getItemId();
            ...
            if(!empty($info['recommend'])){
                $item->setRecommend($info['recommend']);
            }else{
                $item->setRecommend(0);
            }
        }
